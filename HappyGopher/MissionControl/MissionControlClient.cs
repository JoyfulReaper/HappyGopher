...
public sealed class MissionControlClient(
    ...
    IOptions<MissionControlOptions> options,
    ...
    : IMissionControlClient
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            logger.LogWarning(
                "Mission Control rejected event {EventType} with status {StatusCode}",
                eventType,
        ...
        catch (HttpRequestException exception)
        {
            logger.LogWarning(
                exception,
                "Mission Control is unavailable while publishing {EventType}",
                eventType);
        ...
        catch (Exception exception)
        {
            logger.LogError(
                exception,
                "Unexpected Mission Control publishing failure for {EventType}",
                eventType);
    ...
    private sealed record PublishEventRequest<TPayload>(
        Guid EventId,
        string EventType,
        int SchemaVersion,
        DateTimeOffset OccurredAt,
